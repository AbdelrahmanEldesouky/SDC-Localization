...
Eigen::Matrix4d ndt_transform(PointCloudT::Ptr mapCloud,
                              PointCloudT::Ptr source, Pose startingPose) {
  // Initialize NDT
...
int main() {
  ...
  auto lidar_transform =
      cg::Transform(cg::Location(-0.5, 0, 1.8) + user_offset);
  auto lidar_actor =
      world.SpawnActor(lidar_bp, lidar_transform, ego_actor.get());
  ...
  cout << "Loaded " << mapCloud->points.size() << " data points from map.pcd"
       << endl;
  ...
  lidar->Listen([&new_scan, &lastScanTime, &scanCloud](auto data) {
    if (new_scan) {
      ...
      if (pclCloud.points.size() > 5000) { // CANDO: Can modify this value to
                                           // get different scan resolutions
  ...
  while (!viewer->wasStopped()) {
    ...
    if (refresh_view) {
      viewer->setCameraPosition(pose.position.x, pose.position.y, 60,
                                pose.position.x + 1, pose.position.y + 1, 0, 0,
                                0, 1);
      refresh_view = false;
    }
    ...
    renderRay(viewer,
              ...
              Point(truePose.position.x + 4 * cos(stheta),
                    truePose.position.y + 4 * sin(stheta), truePose.position.z),
              "steer", Color(0, 1, 0));
    ...
    if (!new_scan) {
      ...
      // TODO: Find pose transform by using NDT matching
      ...
      Eigen::Matrix4d matching_transform =
          ndt_transform(mapCloud, cloudFiltered, pose);
      ...
      pcl::transformPointCloud(*cloudFiltered, *transformed_scan_ptr,
                               matching_transform);

      viewer->removePointCloud("scan");
      // TODO: Change `scanCloud` below to your transformed scan
      renderPointCloud(viewer, transformed_scan_ptr, "scan", Color(1, 0, 0));

      viewer->removeAllShapes();
      drawCar(pose, 1, Color(0, 1, 0), 0.35, viewer);

      double poseError = sqrt((truePose.position.x - pose.position.x) *
                                  (truePose.position.x - pose.position.x) +
                              (truePose.position.y - pose.position.y) *
                                  (truePose.position.y - pose.position.y));
      if (poseError > maxError)
        maxError = poseError;
      double distDriven = sqrt((truePose.position.x) * (truePose.position.x) +
                               (truePose.position.y) * (truePose.position.y));
      viewer->removeShape("maxE");
      viewer->addText("Max Error: " + to_string(maxError) + " m", 200, 100, 32,
                      1.0, 1.0, 1.0, "maxE", 0);
      viewer->removeShape("derror");
      viewer->addText("Pose error: " + to_string(poseError) + " m", 200, 150,
                      32, 1.0, 1.0, 1.0, "derror", 0);
      viewer->removeShape("dist");
      viewer->addText("Distance: " + to_string(distDriven) + " m", 200, 200, 32,
                      1.0, 1.0, 1.0, "dist", 0);
